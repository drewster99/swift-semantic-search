...
/// Built-in demo corpus for `cli-demo`. Twenty-four short sentences spread evenly
/// across six everyday topics — cooking, software, astronomy, music, fitness, and
/// gardening — chosen so semantic queries that don't share keywords with the
/// documents still rank the right one on top.
///
/// Mirrors `Tests/SemanticSearchTests/Fixtures/corpus.json` but lives here as
/// a Swift literal so the executable target needs no resource plumbing.
enum DemoCorpus {
    struct Document {
        let id: String
        let topic: String
    ...
    /// One-line blurb shown when the user runs with `--demo-corpus` so they
    /// know what kinds of queries to try.
    static let blurb: String = """
        Built-in demo corpus: 24 short sentences across six topics —
        cooking, software, astronomy, music, fitness, gardening.
        Try queries like:
          • "how do I get a really sharp edge on a chef's knife"
          • "stopping data races between threads in Swift"
          • "what causes the northern lights"
          • "interval workout for maximum aerobic capacity"
          • "ratio of browns to greens for healthy compost"
        """
